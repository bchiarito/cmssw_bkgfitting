        ...
        for region in regions:  # loop through twoprong sideband regions
            ...
            for i in range(len(bins)):  # loop through pt bins for a fixed twoprong sideband
                ...
                for eta_reg in eta_regions:  # loop through eta regions for fixed pt-bin and fixed twoprong sideband
                    ...
                    if region == "iso_sym" or region == "iso_asym": old_method = True
                    
                    # THE FOLLOWING APPLIES FOR EGAMMA_2018 ABCD DATASETS
                    # Note: an initial guess must be made each time to eliminate the randomness of MinuE, which could yield bad fits if no guess is provided
                    # THIS SHOULD BE CONVERTED TO A TEXT FILE INPUT SOON
                    # Iso-Sym Guesses
                    if region == "iso_sym":
                        if eta_reg == "barrel":
                            if bins[i] == 20:
